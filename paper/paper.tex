...
live  objects,  both evaluated  (i.e.,  in  WHNF) and  closures.   Our
...
\todelete{While this idea has been shown  to be effective for a first-order {\em
...
expressions).}
We encounter significant challenges in defining a liveness based garbage 
collector for lazy languages. 
\added{
Firstly, during garbage collection the heap contains both fully 
evaluated data and unevaluated thunks/closures. 
For liveness-based
garbage collection to  be effective, we need to extend  it to closures
apart from  evaluated data. Since closures can escape the scope in which 
they were created, they need to carry liveness information of its free 
variables. 
Moreover, since  data  is made  live  by evaluation  of
...
complicates  liveness analysis  itself.  
Also, as  execution progresses and possible  future uses are
eliminated, we  update the  liveness information in  a closure  with a
more precise version.  Because of these reasons, the garbage collector
also  becomes significantly  more  complicated  than a  liveness-based
collector for a eager language.
}

\todelete{
Firstly, since  data  is made  live  by evaluation  of
closures, and  in lazy languages the  place in the program  where this
evaluation  takes  place  cannot be  statically  determined,  laziness
...
free variables. As  execution progresses and possible  future uses are
eliminated, we  update the  liveness information in  a closure  with a
more precise version.  Because of these reasons, the garbage collector
also  becomes significantly  more  complicated  than a  liveness-based
collector for a eager language.
...
\label{sec:motiv}
\input{fig_motiv}   Figure~\ref{fig:mot-example}   shows  an   example
...
\added{
\begin{myitemize}
\item  We propose a liveness based garbage collection scheme for a lazy first order 
functional language and prove its correctness. 
To the best of our knowledge this is the first effort to define a
liveness based garbage collection for a lazy language. 
Our main contribution is the way we handle liveness of functional values i.e closures which 
distinguishes our work from earlier work in this area. 

\item
  We  formulate  a  liveness  analysis for  the  lazy  first-order
  functional language and  prove its correctness. The 
  proof involves specifying liveness through a non-standard semantics  and then proving the analysis  
...
\item 
  ...
  whether   to  copy  a  cell   during  garbage  collection
  translates  to a  membership problem  for such  grammars.  In  this
  paper, we  have shown the  undecidability of this problem. We 
  circumvent the  problem by over-approximating the  grammars
  with DFAs.
\item 
We  have implemented a garbage  collector that uses the  result of liveness analysis to
  retain live  cells.
Our  experiments reveal  interesting space-time
  ...
  show the effectiveness of liveness-based garbage collection.
\end{myitemize}
}
\todelete{
\begin{myitemize}
...
\item The analysis results  in a set  of context-free  grammars along
  ...
  whether   to  copy  a  cell   during  garbage  collection
  translates  to a  membership problem  for such  grammars.  In  this
  paper, we  have shown the  undecidability of this problem and 
  circumvent the  problem by over-approximating the  grammars
  with DFAs.
...
As far  as we know, this  is the first work  on liveness-based garbage
collection      for      lazy       languages.       All      previous
attempts~\cite{shaham01heap,   ran.shaham-sas03,   shaham02estimating,
  asati14lgc, karkare06effectiveness} have  involved either imperative
or eager functional languages.
}
...
Any attempt to dereference a reference \mycomment{that has been declared not live by our analysis} %%with a null-liveness   
...
%% Liveness  analysis  of  $f$  will  result in  not  only  contain  a
%% production  $P$ itself,  but could  also contain  extra productions
%% that necessarily contain the \clazy\ symbol.
...
%%%%% prasanna minefield
\begin{figure*}[b!]
\noindent{\Large \bf A. Minefield semantics(Not included in the paper)}
\begin{center}\footnotesize
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{|c|c|c|}
\hline
...
{\sc prim-add} \\
\hline
\makecell[t]{$\cred{GC(\rho_1, S, \heap_1, (+~x~y), \sigma) = (\rho, S, \heap)}$,\\$\heap(\rho(x)) \mbox{ is } \langle s, \rho'\rangle$} &\makecell[t]{$\rho, S,
  \heap, (+~x~y), \cred{\sigma}$  $\rightsquigarrow$ \\ $ \rho', (\rho, \rho(x),
  (+~x~y), \cred{\sigma})\!:\!S, \heap, s, \cred{\clazy\sigma}$}      &
{\sc prim-1-clo} \\
\hline
\makecell[t]{$\cred{GC(\rho_1, S, \heap_1, (+~x~y), \sigma) = (\rho, S, \heap)}$,\\$\heap(\rho(y)) \mbox{ is } \langle s, \rho'\rangle $} & \makecell[t]{$\rho,
  S, \heap, (+~x~y), \cred{\sigma}$  $\rightsquigarrow$ \\ $ \rho', (\rho,\rho(y),
  (+~x~y), \cred{\sigma})\!:\!S, \heap, s, \cred{\clazy\sigma}$}      &
{\sc prim-2-clo} \\
\hline
\makecell[t]{$\cred{GC(\rho_1, S, \heap_1, (f~\myvec{x}), \sigma) =
    (\rho, S, \heap)}$,\\ $\mathit{f}~\mbox{defined as}$
...
{\sc funcall} \\
\hline
\makecell[t]{$\cred{GC(\rho_1, S, \heap_1, (\LET~x\leftarrow
  ...
  s~\IN~e), \cred{\sigma}$  $ \rightsquigarrow$ \\ $ \rho\oplus[x
    \mapsto \ell], S, \heap[\ell := \langle s, \lfloor\rho\rfloor_{FV(s)}\rangle], e, \cred{\sigma}$} &
{\sc let} \\ 
\hline
\makecell[t]{$\cred{GC(\rho_1, S, \heap_1, (\pi:\SIF~\psi:x~e_1~e_2),
    \sigma) = (\rho, S, \heap)}$, \\
  $\rho(x) \mbox{ is } \bot$} & $\rho, S,
...
\hline
\makecell[t]{$\cred{GC(\rho_1, S, \heap_1, (\pi:\SIF~\psi:x~e_1~e_2),
    \sigma) = (\rho, S, \heap)}$, \\$\heap(\rho(x)) \ne 0$}  & \makecell{$\rho, S, \heap, (\pi:\SIF~\psi:x~e_1~e_2),
  \cred{\sigma}$  $\rightsquigarrow \rho, S, \heap,  e_1, \cred{\sigma}$} & {\sc if-true} \\
\hline
\makecell[t]{$\cred{GC(\rho_1, S, \heap_1, (\pi:\SIF~\psi:x~e_1~e_2), \sigma) = (\rho, S, \heap)}$,\\$\heap(\rho(x)) = 0$} & \makecell{$\rho, S, \heap, (\pi:\SIF~\psi:x~e_1~e_2),
  \cred{\sigma}$   $\rightsquigarrow
\rho, S, \heap,  e_2, \cred{\sigma}$} & {\sc if-false} \\
\hline
\makecell[t]{$\cred{GC(\rho_1, S, \heap_1, (\pi:\SIF~\psi:x~e_1~e_2), \sigma) = (\rho, S, \heap)}$,\\$\heap(\rho(x)) = \langle s, \rho' \rangle $} & \makecell[t]{$\rho, S, \heap,
...
{\sc if-clo} \\
\hline
\makecell[t]{$\cred{GC(\rho_1, S, \heap_1, (\SRETURN~x), \sigma) = (\rho, S, \heap)}$,\\
  $\rho(x) \mbox{ is } \bot$} & $\rho, S,
  \heap, (\SRETURN~x), \cred{\sigma} \rightsquigarrow \bang$   &
{\sc return-bang} 
\\
\hline
\makecell[t]{$\cred{GC(\rho_1, S, \heap_1, (\SRETURN~x), \sigma) =
    (\rho, S, \heap)}$,\\ $\heap(\rho(x))~\mbox{is in WHNF with
    value}~v$} & \makecell[t]{$\rho,
  (\rho', \ell, e, \cred{\sigma'})\!:\!S, \heap, (\SRETURN~x), \cred{\sigma}$  $\rightsquigarrow$\\$ \rho', S, \heap[\ell := v], e, 
  \cred{\sigma'}$} &
{\sc return-whnf}\\
\hline
\makecell[t]{$\cred{GC(\rho_1, S, \heap_1, (\SRETURN~x), \sigma) = (\rho, S, \heap)}$,\\$\heap(\rho(x)) = \langle s, \rho' \rangle $} & \makecell[t]{$\rho, S, \heap,
