...
\added{
\begin{itemize}
\item {\color{Myblue}We propose a liveness based GC scheme for a lazy
  first order functional language and prove its correctness.
  Thomas~\cite{Thomas19951} describes a copying garbage collector for
  Three Instruction Machine (TIM) that only traces live variables in a
  closure's environment, i.e., the variables that are useful for
  evaluating the closure.  Their notion of liveness of variables is
  close to the classical notion of liveness, and does not take into
  account liveness of parts of structures (say, only the spine of a
  list).  To the best of our knowledge, it is the only earlier work on
  GC that takes liveness for closures into account.  All previous
  ...
  approach is in handling of liveness of closures, an issue that
  arises due to laziness at runtime.}
...
For   the   running  example   (Figure~\ref{fig:mot-example2-a}),   if
...
as a start  symbol. The grammar for  \var{\Df{\length}{1}} and \var{\sigma_{\length}} comprise of the following productions:
