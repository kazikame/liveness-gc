...
The concept of demand and liveness for lazy languages are almost similar to their 
counterparts in the eager language. The important distinction comes from the fact 
that in eager languages, variables can point only to data values (either atomic values or cons cells). 
Hence any demand on the variable can easily be transferred onto the data being pointed. 
In lazy languages, a variable can either point to a fully evaluated data value or it 
...
to transfer the demand onto the \CDR~ part of \px~ as it still does not exist. In such a case 
we treat the closure as a demand transformer and tranform the demand on \py~ to the arguments 
of the closure pointed by \px. This procedure is similar to the process of transferring the demand 
from the return value of a function to the arguments of the function call in an eager language. In case 
of lazy languages this process cannot be done statically as we cannot statically determine whether a variable points to 
a data value or a closure. Our method approximates the liveness of closure arguments at runtime 
by tagging extra information with each closure. Since the arguments of the closures could themselves be closures in turn, 
we keep recursively applying the same procedure until we hit a data value. The only case where we do not 
