...
\begin{figure}[p]
\begin{tabular}{@{}c@{}c@{}}
\hskip -4mm{\epsfig{file=sudoku.eps, height=4cm, width=4.10cm}}
&
{\epsfig{file=lcss.eps, height=4cm, width=4.10cm}}
\\
\hskip -4mm{\epsfig{file=gc_bench.eps, height=4cm, width=4.10cm}}
 &
{\epsfig{file=nperm.eps, height=4cm, width=4.10cm}}
\\
\hskip -4mm{\epsfig{file=fibheap.eps, height=4cm, width=4.10cm}}
&
{\epsfig{file=fibheap.eps, height=4cm, width=4.10cm}}%%Add knightstour.eps
\\
\hskip -4mm{\epsfig{file=nqueens.eps, height=4cm, width=4.10cm}}
&
{\epsfig{file=treejoin.eps, height=4cm, width=4.10cm}}
\end{tabular}
 \caption{Memory  usage  of programs.  The  blue  and the  red  curves
   indicate the number of cons cells  in the active semi-space for LGC
   and  RGC respectively.  The black  curve represents  the number  of
...
\hline
  ...
  H, \kappa, \sigma, e_f$ \\ $\rightsquigarrow \rho', S, H[\rho'(x) :=
    \kappa], e, \sigma', e_f'$ }   &  \sc{const}
\\
\hline
          & \makecell{$\rho, (\rho', z, e, \cred{\sigma',
    e_f'})\!:\!S, H, (\CONS~x~y), \sigma, e_f$ \\ $\rightsquigarrow
  \rho', S, H[z := (H(\rho(x)),H(\rho(y)))], e, \sigma', e_f'$}     &  \sc{cons} \\
\hline
$H(\rho(x)) \mbox{ is } (d_1, d_2)$ & \makecell{$\rho, (\rho', z, e,
  \cred{\sigma', e_f'} )\!:\!S, H, (\CAR~x), \sigma, e_f$ \\ $
  \rightsquigarrow \rho', S, H[\rho(z) := d_1], e, \sigma', e_f'$}      &
\sc{car-whnf} \\
\hline
$H(\rho(x)) \mbox{ is } \langle s, \rho'\rangle$ &\makecell{ $\rho, S,
  H, (\CAR~x), \sigma, e_f$ \\ $\rightsquigarrow \rho', (\rho, x,
  (\CAR~x), \cred{\sigma, e_f})\!:\!S, H, s, (\clazy \cup \acar)\sigma , \_ $}      &
\sc{car-clo}
\\
\hline
$H(\rho(x)), H(\rho(y)) \in \mathbb{N}$
 & \makecell{$\rho, (\rho', z, e, \cred{\sigma', e_f'})\!:\!S, H,
  (+~x~y), \sigma, e_f$ \\  $\rightsquigarrow \rho', S, H[\rho(z)
    \mapsto H(\rho(x)) + H(\rho(y))], e, \sigma', e_f'$}      &
\sc{prim-whnf} \\
\hline
$H(\rho(x)) \mbox{ is } \langle s, \rho'\rangle$ &\makecell{$\rho, S,
  H, (+~x~y), \sigma, e_f$ \\ $\rightsquigarrow \rho', (\rho, x,
  (+~x~y), \cred{\sigma, e_f})\!:\!S, H, s, \clazy\sigma, \_$}      &
\sc{prim-1-clo} \\
\hline
$H(\rho(y)) \mbox{ is } \langle s, \rho'\rangle $ & \makecell{$\rho,
  S, H, (+~x~y), \sigma, e_f$ \\ $\rightsquigarrow \rho', (\rho, y,
  (+~x~y), \cred{\sigma, e_f})\!:\!S, H, s, \clazy\sigma, \_ $}      &
\sc{prim-2-clo} \\
\hline
{$\mathit{g}~\mbox{defined as}$
$~(\DEFINE~(g~\myvec{y})~e_{\mathit{g}})$}  & \makecell{$\rho, S, H,
  (g~\myvec{x}), \sigma, e_f$ \\ $\rightsquigarrow [\myvec{y} \mapsto
    \rho(\myvec{x})], S, H, e_{\mathit{g}}, \sigma, e_g$}      &
\sc{funcall} \\
\hline
$\ell$ is a new location& \makecell{$\rho, S, H, (\LET~x\leftarrow
  s~\IN~e), \sigma, e_f$ \\ $ \rightsquigarrow \rho\oplus[x \mapsto \ell], S, H[\ell \mapsto \langle s, \lfloor\rho\rfloor_{FV(s)}  \oplus [x \mapsto
  \ell]\rangle], e, \sigma, e_f$} &
\sc{let} \\
\hline
$H(\rho(x)) \ne 0$ & \makecell{$\rho, S, H, (\pi:\SIF~\psi:x~e_1~e_2),
  \sigma, e_f$ \\ $\rightsquigarrow \rho, S, H,  e_1, \sigma, e_f$} & \sc{if-true} \\
\hline
$H(\rho(x)) = 0$ & \makecell{$\rho, S, H, (\pi:\SIF~\psi:x~e_1~e_2),
  \sigma, e_f$ \\   $\rightsquigarrow
\rho, S, H,  e_2, \sigma, e_f$} & \sc{if-false} \\
\hline
$H(\rho(x)) = \langle s, \rho' \rangle $ & \makecell{$\rho, S, H,
  (\pi:\SIF~\psi:x~e_1~e_2), \sigma, e_f$ \\ $\rightsquigarrow
\rho', (\rho, x, (\SIF~x~e_1~e_2),  \sigma, e_f)\!:\!S, H, s,
\clazy\sigma, e_f$}
&
\sc{if-clo} \\
\hline
{$H(\rho(x))~\mbox{is}$ $\mbox{whnf with value}~v$}& \makecell{$\rho,
  (\rho', z, e, \cred{\sigma', e_f'})\!:\!S, H, (\SRETURN~x), \sigma,
  e_f$ \\ $\rightsquigarrow \rho', S, H[\rho(z) \mapsto v], e,
  \sigma', e_f'$} &
\sc{return-whnf}\\
\hline
$H(\rho(x)) = \langle s, \rho' \rangle $ & \makecell{$\rho, S, H,
  (\psi:\SRETURN~x), \sigma, e_f$ \\ $
  \rightsquigarrow$
