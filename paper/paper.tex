...
garbage  collector    to  efficiently  reclaim  memory,  empirical
studies on Scheme~\cite{karkare06effectiveness} and, more importantly,
on  Haskell~\cite{rojemo96lag}  programs  have shown  that  garbage collectors  leave
...
\warning{Experiments  with  a   single  generation  copying  collector
  ...
  number of collections up to ???\% and a decrease in the
  minimum memory requirement up to ???\%. 
  As an added benefit, there is also a reduction in the overall
  execution time for some programs.}
...
%% Given a state $\rho, \stk, \heap,
%% e$  of  the  transition system, we shall call the pair $(\rho, e)$ as
%% the    \emph{current
%%   evaluation  context}, and for a continuation frame $(\rho',  \ell, e')$ in
%% \stk, we shall call the pair $(\rho', e')$ as a   
%% \emph{suspended  evaluation  context}.}
%% %% \warning{The
...
touched  per garbage collection,  total  number of  dragged cells  and  total time  to
...
dragged cells significantly. However, the cost  for LGC is that per garbage collection
...
infeasible  to   incorporate  liveness  based   garbage collection  for
closures in a practical garbage collectors.
...
and shown its benefit for  standard benchmark programs.  This was done
