...
{\color{Myblue}  As noted  earlier, $x.\alpha  \in \Lv$  specifies the
...
Section~\ref{sec:liveness-analysis}. {\color{Myblue} It is easy to see that the analysis
...
\item      We     augment      the      standard     semantics      in
  ...
  of  each \LET\  expression.  {\color{Myblue}Note  that, unlike  eager
...
\end{tabular}
\end{center}
\caption{Minefield  semantics for  {\sc  car-bang}  and {\sc  car-clo}
...
\item  $GC(\rho,  S,  \heap,  e, \sigma)$ models  a  liveness-based  garbage
  ...
  To do so, $GC$ needs to consider the following environments:  (1) the  environment in  the
...
{\color{Myblue}
 ...
 \item \label{env}  For each of  these environments, $GC$  calculates a
   liveness  environment $\Lv$  using  the  corresponding  $e$  (or  $s$)  and
   $\sigma$.
 \item 
   For each
    location $\ell$,  $GC$  sets $\heap(\ell)$  to $\bot$ iff for each
