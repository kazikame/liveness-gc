...
\added{
\begin{itemize}
\item {\color{Myblue}We  propose an interprocedural liveness  based GC
  ...
  the  Three  Instruction  Machine  (TIM)  that  only  preserves  live
  closures in a function's environment (also called a frame). However,
  in the absence of  details, it is not clear whether  a) the scope of
  the method is interprocedural, and b) it handles algebraic datatypes
  like lists (the original design of TIM did not).  All other previous
  attempts~\cite{shaham01heap,  ran.shaham-sas03,  shaham02estimating,
    asati14lgc, karkare06effectiveness} involved  either imperative or
