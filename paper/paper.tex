...
language syntax) with the program point just before expression
$e$. 
...
%% To   deal  with   this,   we  introduce   an   analysis  which   is
%%flow-insensitive but path-sensitive\comment{Checked the terminology,
%%path-sensitive means certain properties  holding along a path only,
%%http://en.wikipedia.org/wiki/Data-flow\_analysis} into our analysis.
...
\langle\pi_1, \pi_2, \pi_3, \pi_4, \psi_2\rangle$ and  $ep_3 = \langle \pi_1, \pi_2,
\pi_5,  \pi_6, \pi_7, \pi_8, \psi_3\rangle$.

  
Given  a function  $\mathit{f}$ and  a demand  $\sigma$,  the  liveness
analysis computes two maps:  the function $\mathcal{L}$ computes a map
from a  e-path to a  liveness environment arising  out of all  uses of
variables  along   the  path.  We  call   this  map  $\cal{M}_{\mathit
  f}^{\sigma}$ (or  $\cal{M}_{\mathit f}$ when the  demand $\sigma$ is
clear from  the context).  A  second map $\cal{P}_{\mathit f}$  maps a
program point to  the set of e-paths sharing the  point.  If a program
point happens  to fall on more  than one such path,  then the liveness
...
is created  and added to the  union.  Also notice  a consequences of
laziness---the  entire  expression  including  the  condition  is  not
...
\end{figure*}
%======================================================================================
%% \mycomment{In an eager language, \pw\ would not have been live after $\pi_7$, the
...
$\sigma$,  we  get an  equation  for  \Df{f}{i} that is
...
To avoid analysing the body of a function we assumed a symbolic demand $\sigma$
and created a summary liveness for the function arguments and the variables 
in the function. To get the actual liveness we have to substitute the actual 
demands that are placed on the function. We calculate an over-approximation 
of the actual demand, a {\em summary demand} and calculate the liveness at each GC point inside the 
function. Even though we lose a bit of precision due to this, our 
experiments show that this is acceptable \comment{Is this claim true?}.
The {\em summary demand} is calculated by taking a union of the demand at 
every call site of a function. A function call in our language can only be of the
form $\pi\!\!:\!(\LET~x  \leftarrow (f\,y_1\,\ldots\,y_n)\ldots$. The demand on 
$f$ due to this function call will be the liveness of $x$ at the program 
point $\pi$. Denoting this demand as $\deltacall{f}{\pi_i}{g}$  for this demand  (contributed by call
site $\pi$  in function $g$) we can write the {\em summary demand} as 
$\sigma_f =  \deltacall{f}{\pi^1}{g^1} \cup \cdots
...
\cup \deltacall{f}{\pi^k}{g^k}$Rewrite, copied from earlier paper}.
