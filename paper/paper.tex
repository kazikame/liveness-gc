  ...
  \begin{tabular}{cc}
    \begin{boxedminipage}{.51\textwidth}
	  ...
	  \UNL{2}   \hspace*{.78cm}   $\pi_9\!\!:\,      $ (\LET\ \pb\  $\leftarrow$  \ldots \IN 
          \UNL{3}   \hspace*{.78cm}    $\pi_{10}\!\!:\, $(\LET\ \pw\  $\leftarrow$  (\length\ \pb) \IN~$\pi_{11}\!\!:\,$ (\RETURN$_{ep_4}$\ \pw)))))))
\end{uprogram}}
        } 
    \end{boxedminipage}
    & 
      ...
      {\sf
        ...
        \renewcommand{\arraystretch}{1}{
          \begin{uprogram}
            \UNL{1} (\DEFINE\ (\length~\xl)
            \UNL{2}  $\pi_1\!\!:\, $(\LET\ \px\ $\leftarrow $\ (\NULLQ~\xl) \IN
            \UNL{3} \hspace*{.05cm} $\pi_2\!\!:\,$(\SIF\ $\psi_3\!\!:\,$ \px
            \UNL{4} \hspace*{1.15cm} $\psi_1\!\!:\, (\RETURN~0)$
            \UNL{4} \hspace*{1.15cm}    $\pi_3\!\!:\, $(\LET~\pu\  $\leftarrow$  (\CDR~\xl)  \IN
            \UNL{5} \hspace*{1.15cm}   $\pi_4\!\!:\, $(\LET~\py\  $\leftarrow$  (\length~\pu)  \IN
            \UNL{6} \hspace*{1.15cm} $\psi_2\!\!:\, (\RETURN~(1~+~\py))$)))))
        \end{uprogram}}
        \renewcommand{\arraystretch}{1}{
	  \begin{uprogram}
	  \UNL{1} $\pi_\mainpgm\!\!:\, \pi_5\!\!:\, $(\LET\  \pa\  $\leftarrow$ \ldots  \IN
	  \UNL{2}   \hspace*{.78cm}   $\pi_6\!\!:\,      $ (\LET\ \pb\  $\leftarrow$  \ldots \IN 
          \UNL{3}   \hspace*{.78cm}    $\pi_{7}\!\!:\, $(\LET\ \pw\  $\leftarrow$  (\length\ \pb) \IN~$\psi_{4}\!\!:\,$ (\RETURN\ \pw)))))))
\end{uprogram}}
        } 
    \end{boxedminipage}
    & 
    \begin{boxedminipage}{.49\textwidth}
      {\sf
        ...
        $\cal{M_{\mbox{\tiny \length}}} = $ 
            \begin{tabular}{lc}            
              $[$
              $\psi_2 \mapsto \lbrace \py.\epsilon, \pu.\Lf{\length}{\mbox{1}}{\epsilon}, \
              \xl.\mbox{1}\Lf{\length}{\acdr}{\epsilon}, \xl.\epsilon  \rbrace ,$ \\ 
              $\psi_1 \mapsto \lbrace \rbrace , $\\
              $\psi_3 \mapsto \lbrace \px.\epsilon, \xl.\epsilon \rbrace $  \\
              $]$
            \end{tabular} \\
            $\cal{P_{\mbox{\tiny \length}}} = [$
              \begin{tabular}{lc}
                ...
                $ \pi_{\mbox{3}} \mapsto \lbrace \psi_3 \rbrace, $ \
                $ \pi_{\mbox{4}} \mapsto \lbrace \psi_1 \rbrace, $ \
                $ \pi_{\mbox{5}} \mapsto \lbrace \psi_2 \rbrace, $ \\
                $ \pi_{\mbox{6}} \mapsto \lbrace \psi_2 \rbrace, $ \
                $ \pi_{\mbox{7}} \mapsto \lbrace \psi_2 \rbrace $ \\
                $]$
              \end{tabular}\\
              $\cal{M_{\mbox{\tiny \main}}} = $ 
              \begin{tabular}{lc}            
              ...
              $\cal{P_{\mbox{\tiny \main}}} = [$
                \begin{tabular}{lc}
                  ...
                  $ \pi_{\mbox{11}} \mapsto \lbrace \psi_4 \rbrace, $ \
                  $ \ldots  $ \
                  $]$  
                \end{tabular}
        \end{tabular}
...
$\emptyset$, if $\sigma$ itself $\emptyset$. This creates difficulties
since  we  want  to  solve  the  equations  arising  out  of  liveness
symbolically, and realizing these rules requires $\sigma$ to be known.
...
equations  of the  form $\Lf{\mathit{f}}{i}{\sigma}  = \Lanv{1}{x_i}$,
where  $x_i$   is  the  $i^{\mbox{\footnotesize   th}}$  parameter  of
$\mathit{f}$. Doing this for  \length, we get:
\begin{align*}
 \Lanv{1}{\xl} &= \cbar\sigma \cup \acdr\Lf{\length}{1}{\sigma}
\end{align*}
...
case  of \length,  \Lan{1}\ may,  in turn,  be expressed  in  terms of
\Lfone{\mathit{f}}.
...
get:
  ...
  \end{align*}
  Note that these equations can also be viewed as CFGs.
