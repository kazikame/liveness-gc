      ...
      {\sf
        ...
        \renewcommand{\arraystretch}{1}{
          \begin{uprogram}
            ...
            \UNL{4} \hspace*{1.15cm} $\pi_4\!\!:\, (\RETURN_{ep_1}~0)$
            \UNL{4} \hspace*{1.15cm}    $\pi_5\!\!:\, $(\LET~\pu\  $\leftarrow$  (\CDR~\xl)  \IN
            \UNL{5} \hspace*{1.15cm}   $\pi_6\!\!:\, $(\LET~\py\  $\leftarrow$  (\length~\pu)  \IN
            \UNL{6} \hspace*{1.15cm} $\pi_7\!\!:\, (\RETURN_{ep_2}~(1~+~\py))$)))))
        \end{uprogram}}
        \renewcommand{\arraystretch}{1}{
	  \begin{uprogram}
	  \UNL{1} $\pi_\mainpgm\!\!:\, \pi_8\!\!:\, $(\LET\  \pa\  $\leftarrow$ \ldots  \IN
	  \UNL{2}   \hspace*{.78cm}   $\pi_9\!\!:\,      $ (\LET\ \pb\  $\leftarrow$  (\CAR~\pa) \IN 
          \UNL{3}   \hspace*{.78cm}    $\pi_{10}\!\!:\, $(\LET\ \pw\  $\leftarrow$  (\length\ \pa) \IN~$\pi_{11}\!\!:\,$ (\RETURN$_{ep_4}$\ \pw)))))))
\end{uprogram}}
        } 
    \end{boxedminipage}
    & 
      ...
      {\sf
        ...
        $\cal{M_{\mbox{\tiny length}}} = $ 
            ...
            $\cal{P_{\mbox{\tiny length}}} = [$
              ...
              \begin{tabular}{lc}            
                $[$
                  $ep_4 \mapsto \lbrace \pw.\sigma_{\mathit{\!all}}, \pa.\Lf{\length}{\mbox{1}}{\sigma_{\mathit{\!all}}}, \ldots  \rbrace $ \
                  $]$
              \end{tabular} \\
...
For the running example, containing unique GC points (corresponding to unique {\em e-paths} sets) $ep_1, ep_2, ep_3$ in $\length$ and
$ep_4$ in $e_\main$, this gives

\begin{align*}
\Lanv{7}{\xl} &= \acdr\Lf{length}{1}{\bX\sigma_{\mathit{length}}} \cup \bX\sigma_{\mathit{length}} \\
\Lanv{7}{\pu} &= \Lf{length}{1}{\bX\sigma_{\mathit{\length}}} \\
\Lanv{7}{\py} &= \bX\sigma_{\mathit{length}} \\
\Lanv{3}{\xl} &= \bX\sigma_{\mathit{length}} \\
\Lanv{3}{\px} &= \bX\sigma_{\mathit{length}}
\\[1ex]
...
executes.
\subsection{GC points and automata sharing}
% GC points - mention reduction of gc points due to e-paths
We describe a simple outline of the GC algorthm in figure {\em add refernce to algorithm}.
In a lazy language each \LET~ expression creates a closure, and hence allocates memory 
on the heap. Therefore in contrast to an eager language, every \LET~ statement is a  potential GC point. 
As a GC can be triggered at any of the \LET~ expressions, we  need  to store  liveness  automata  
corresponding  to every  \LET~ statement in the program. 
Even though this leads  to an explosion in the number of
automata, we can optimize it using our observation that  along an {\em E-path} all program
points have  the same liveness. We therefore create a single liveness environment corresponding to each 
unique {\em E-path} sets and share the same automata  for all  program points
having a common {\em E-path} set. In the  example program {\em refer to the example program}, we only need to create
automata for the {\em E-path} sets \ldots, since they are  the only distinct
{\em E-path} sets in the program.
...
\subsection{Liveness based GC algorithm}
% Liveness for cons cells & reachability for closures
When ever the program runs out of memory, we scan the current buffer and we check if 
each reference is live or not and using a depth first traversal copy its children recursively.
The handling of a heap cell depends on the type of data it stores, as described in the algorithm{\em add reference to algorithm}.
\begin{itemize}
\item An atomic value is copied if it is live.
\item A \CONS~ cell is copied if it is live. Its \CAR~ and \CDR~ part are copied recursively if they are live.
\item A closure and everything reachable from it are copied if it is live otherwise nothing is copied. 
\end{itemize}

Observe that we are using liveness based GC for \CONS~ cells whereas we do a reachability based GC for closures. 
Since we do not nullify references that are not live this can lead to copying of incorrect references. 
Consider a \CONS~ cell {\em c} whose \CAR~ part is live but \CDR~ part is not live. During GC the pointer corresponding to the 
\CDR~ part is neither copied and nor nullified. Post GC if this cell becomes part of a closure then during a subsequent GC we will do 
a reachability based GC on {\em c} and hence try to chase the pointer corresponding to its \CDR~ part. 
One way to avoid this problem is by setting references which are not live to null. Doing this during GC is expensive as it 
requires an alias analysis. We avoid this overhead by waiting for the GC to end and then setting any references which point to 
the dead buffer to null. From the definition of liveness any reference which is declared not live will not be referenced beyond that 
execution point till the end of the execution. Hence once the GC is complete any pointer in the live buffer which points to a location 
in the dead buffer can be nullified safely. Although this involves scanning the entire live buffer again, the overhead is not too bad as 
the expected number of live heap cells in the live buffer (immediately after a GC) is usually much less compared to the size of the live buffer. Also, by 
copying closures using reachability we can perform certain optimizations which will not be possible if we use only liveness  based GC.
We describe this optimization in section{\em add reference}.
...
\subsection{Printing and GC on the print stack}
