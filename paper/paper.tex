...
s, \rho'\rangle), (\CAR~x),\cred{\sigma} )\!:\!S,$\\ $\quad \heap,\, s, \, \cred{\sigma}$ }     &
{\sc car-1-clo} 
\\
\hline
...
\makecell[t]{ $\rho, S, \heap, (\CAR~x), \cred{\sigma}
  \rightsquigarrow$ \\
  $\rho', (\rho, \rho(x), (\CAR~x), \cred{\sigma})\!:\!S,$\\ $\quad \heap, s,\cred{\renewcommand{\arraystretch}{1}\left\{
    \begin{array}{lr}
...
\makecell[t]{$\cred{GC(\rho_1, S_1, \heap_1, (\LET~x\leftarrow
    s~\IN~e), \sigma)}$\\ $\cred{ = (\rho, S, \heap)}$,\\$\ell$ is a new location}& \makecell[t]{$\rho, S, \heap, (\LET~x\leftarrow
