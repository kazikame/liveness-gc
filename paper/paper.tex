...
{\color {Myblue}Rewrite  after the  experiments: Experiments
  with  a single  generation  copying collector  confirm the  expected
  performance  benefits.   Liveness-based  collection  results  in  an
  increase  in  garbage  reclaimed.   As a  consequence,  there  is  a
  decrease in  the number of  collections and a  (some quantification)
  decrease in the memory size  required to run programs. Besides there
  is a reduction in the overall  execution time for some programs. Our
  experiments further show that a mixed strategy, where we do liveness
  based GC  up to the closure  boundary and reachability based  GC for
  the  arguments (free  variables) of  the closure,  gives us  similar
  benefits as  a fully liveness based  GC (where the arguments  of the
  closure are also collected using liveness) at much less overhead (?).
}
...
%      \includegraphics[width=.45\textwidth]{motiv-example}
      ...
      \begin{tabular}[t]{p{0.9\columnwidth}}
        ...
        stops at edges marked $\times$ during garbage collection.
      \end{tabular}
    \end{tabular} 
  \end{pspicture}
  \vspace*{-3ex}
...
cell is  preserved during garbage  collection only if it  is reachable
from the root set  through a path of live edges.   All other cells can
be reclaimed.  Thus if a garbage  collection takes place at $\pi$ with
the  heap shown  in Figure~\ref{fig:mot-example}(b),  a liveness-based
collector  (LGC) will  preserve only  the cells  referenced by  $\pz$,
$(\CDR~ \pz)$ and  (the cells constituting) the  closure referenced by
$(\CDR~(\CDR~\pz))$.   In  contrast,  a  reachability-based  collector
(RGC) would have preserved all cells.
...
{
\color {Myblue}[I don't think this is valid anymore!!]
...
to the program points  which trigger evaluation---\SIF\ conditions and
\SRETURN\     expressions.      We     call    such     paths     {\em
  e-paths}.\footnote{Program  points  which   trigger  evaluation  are
  labelled with $\psi$. By an abuse of notation, we use the same label
  $\psi$ to refer  to the e-path terminating at a  point $\psi$.}  For
example, for the function \length, there  are three such paths $\epath_1 =
\langle  \pi_1, \pi_2,  \psi_1\rangle$, $\epath_2  = \langle\pi_1,  \pi_2,
\pi_3, \pi_4, \psi_2\rangle$ and $\epath_3  = \langle \pi_1, \pi_2, \pi_5,
\pi_6, \pi_7, \pi_8, \psi_3\rangle$.
...
evaluation  of   $x$  and  hence  a  new   map  $[\mathit{\epath}  \mapsto
  \{x.\sigma\}] $  needs to  be created, where the $\epath$ is a fresh
...
\begin{eqnarray*}
  ...
  \clazy\sigma_{\length} \mid \acdr \var{\Df {\length}{1}}{\clazy\sigma_{\length}}  \\
  & & \mid
...
simplification  is  continued  till  a fixed  point  is  reached.  The
algorithm for this appears in~\cite{asati14lgc}, Figure[].  Finally we
replace the  $\clazy$ edges by  $\epsilon$ and check if  the automaton
has  valid path  (consisting  of only  \acar\ and  \acdr)  to a  final
state. If it has a valid path,  we remove the \clazy\ edge and mark the
source node as  final, else we remove the \clazy\  edge without marking
the source  node as final.   Once the  \clazy\ edges are  eliminated we
convert the automaton into a deterministic automaton.

\begin{figure}[t!]
  \begin{tabular}{cc}
\psset{unit=1mm,nodesep=0mm,labelsep=0.5mm}
\begin{pspicture}(0,-5)(40,12)
  ...
  \nccurve[angleA=45, angleB=135, ncurv=4, nodesep=-.5]{t1}{t1}
  \putnode{l1}{t1}{0}{10}{\acdr/\clazy}
  \nccurve[angleA=45, angleB=135, ncurv=3, nodesep=-.5]{t2}{t2}
  \putnode{l2}{t2}{0}{10}{\clazy}
\end{pspicture}
&
\psset{unit=1mm,nodesep=0mm,labelsep=0.5mm}
\begin{pspicture}(0,-5)(40,12)
  ...
  \ncline{t0}{t1}
  \nccurve[angleA=45, angleB=135, ncurv=4, nodesep=-.5]{t1}{t1}
  \putnode{l1}{t1}{0}{10}{\acdr}
\end{pspicture}
\\
(a) & (b) \\
\psset{unit=1mm,nodesep=0mm,labelsep=0.5mm}
\begin{pspicture}(0,-5)(40,12) %\psframe(0,-5)(40,12)
  \putnode{t0}{origin}{5}{0}{\Lanv{9}{\pa}}
  ...
  \nccurve[angleA=55, angleB=125, ncurv=3, nodesep=-.5]{t3}{t3}
  \putnode{l2}{t3}{0}{10}{\acdr}
\end{pspicture}
&
\psset{unit=1mm,nodesep=0mm,labelsep=0.5mm}
\begin{pspicture}(0,-5)(40,12) %\psframe(0,-5)(40,12)
  \putnode{t0}{origin}{5}{0}{\Lanv{9}{\pa}}
  \putnode{t1}{t0}{9}{0}{\pscirclebox{\mbox{\ \ \ \ }}} \hspace{5mm}
  \putnode{t3}{t1}{12}{0}{\pscirclebox[doubleline=true]{\mbox{\ \ \ \ }}}
  \psset{arrows=->}
  \ncline{t0}{t1}
  \ncline{t1}{t3}
  \putnode{l1}{t1}{5}{2}{\bcar}
  \nccurve[angleA=55, angleB=125, ncurv=3, nodesep=-.5]{t3}{t3}
  \putnode{l2}{t3}{0}{10}{\acdr}
\end{pspicture}
\\
...
\caption{(a) The  grammar rules for $\langle  \Df{\length}{1} \rangle$
  converted into an automaton, and simplified in (b). (c) and (d) show
  the   automata  for   \Lanv{9}{\pa}.   }\label{fig:example-automata}
\figrule
\end{figure}
...
\end{algorithm}
Algorithm~\ref{algo:lgc-a}  describes  a  liveness-based  garbage  collection
...
\end{algorithm}
  Since the first  algorithm has  significant bookkeeping
...
and copy everything  that is reachable. Algorithm~\ref{algo:lgc-b}  describes  this scheme. 
...
Using  reachablity  for  closures   avoids  the  problem  of  multiple
explorations from the  same cell.  This is done by  maintaining a flag
in each cell to indicate whether it was copied using reachability.
