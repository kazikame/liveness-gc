...
\begin{itemize}
\item State of the operational semantics will be $\rho, S, H, e,
  \sigma$.
\item Stack content will be $\rho, x, e, \sigma, eps$.
\item Liveness environment calculation:
  \begin{enumerate}
  \item Of the   current expression being
  evaluated:\\  
  $ \Lv = \bigcup_{ep \in \mathcal{P}(e)} \mathcal{M}_\sigma(ep)$, where $
  \mathcal{M}_\sigma = \mathcal{L}(e, \sigma, \Lfonly)$
\item Liveness environment of a stacked computation\\
  $ \Lv = \bigcup_{ep \in eps} \mathcal{M}_\sigma(ep), \text{where }
  \mathcal{M}_\sigma = \mathcal{L}(e, \sigma, \Lfonly)$ 
  \end{enumerate}
...
$GC(\rho, S, H, L, SL) = (\rho', S', H')$ 
  ...
  \item For each $x \in domain(\rho)$,  $\rho'(x)=\bot$ iff $x.\epsilon
    \notin \Lv$.
\item For each $x \in domain(\rho)$ where $(\rho,\_,\_,\_,\_,\_)$ is a
  stack entry  with \Lv\ as the  liveness environment, $\rho'(x)=\bot$
  iff $x.\epsilon \notin \Lv$.
