  ...
  \begin{tabular}{cc}
    \begin{boxedminipage}{.5\textwidth}
      {\sf
        \renewcommand{\arraystretch}{1}{
          \begin{uprogram}
            ...
            \UNL{3} \hspace*{.05cm} $\psi_1\!\!:\,$(\SIF\ $\pi_3\!\!:\,$ \px
            \UNL{4} \hspace*{1.15cm} $\pi_4\!\!:\, (\RETURN_1~0)$
            \UNL{4} \hspace*{1.15cm}    $\pi_5\!\!:\, $(\LET~\py\  $\leftarrow  (\CDR~\xl)$  \IN
            \UNL{5} \hspace*{1.15cm}   $\pi_6\!\!:\,
            $(\LET~\pz\  $\leftarrow  (\length~\py)~\IN$
            \UNL{6} \hspace*{1.15cm}  $\pi_7\!\!:\,
           $(\LET~\pw\  $\leftarrow  (\PRIM~1~\pz)~\IN$
            \UNL{7} \hspace*{.05cm}\hspace*{1.15cm} $\psi_2\!\!:\, (\RETURN_1~\pw)))))))$
        \end{uprogram}}
	  ...
	  \UNL{2}   \hspace*{.78cm}              (\LET\ \pb\  $\leftarrow$  \ldots \IN 
          ...
          $(\LET\ \pc\  $\leftarrow (\length\ \pb)~\IN$
          \UNL{4}\hspace*{.05cm}\hspace*{.78cm} $\psi_{3}\!\!:\, (\RETURN\ \pc)))))))$
\end{uprogram}}
        } 
    \end{boxedminipage}
    & 
...
\acar\ and \acdr\ from  access paths in $\sigma$.  Mathematically this
is fine but  causes problems when solving the  liveness equations {\em
  symbolically}  since such  decomposition  cannot be  expressed as  a
context-free  grammar.  Similarly,  the rules  for \CAR,  \CDR, \PRIM,
\NULLQ,  and \SIF\  require  us  to return  $\emptyset$,  on the  null
demand. While this is mathematically fine, for our grammar formulation
we would like to treat these operations symbolically.




To  handle  this,  we  first  introduce  three  new  symbols.
...
\begin{proposition}
Assume that a liveness computation based on the original set
of rules gives the liveness of the variable $x$ at a program
point  $\pi_i$   as  $\sigma$  (symbolically,  $\Lanv{i}{x}=
\sigma$).   Further,  let  $\Lanv{i}{x}= \sigma'$  when  the
modified rules are used instead of \Lfunonly.  Then $\sigma'
...
\alan{For practical use we need to solve the liveness equations {\em finitely and
...
calling contexts (e.g.\  to a single unified context  in 0-CFA style).
However, it  turns out that we  can solve the  equations for $\Lfonly$
symbolically  without  this  reduction,  so  here  we  defer  this  to
Section~\ref{sec:bodylivenessbodies}.    (It    also   allows   easier
extension to dynamically determined liveness---future work.)
%
...
The reflexive  transitive closure of  $\hookrightarrow$ will
be   denoted    as   $\stackrel{*}{\hookrightarrow}$.    The
following proposition relates \Lfunonly\ and \Lfunonly$'$:


\begin{proposition}
Assume that a liveness computation based on \Lfunonly\ gives
the liveness of the variable  $x$ at a program point $\pi_i$
as $\sigma$ (symbolically, $\Lanv{i}{x}= \sigma$).  Further,
let $\Lanv{i}{x}= \sigma'$ when \Lfunonly$'$ is used instead
of  \Lfunonly.  Then  $\sigma' \stackrel{*}{\hookrightarrow}
\sigma$.
\end{proposition}

To  see why the  proposition is  true, consider  an analysis
based  on  \Lfunonly$'$ in  which  $\sigma$  appears in  the
context  $\mathit{ref\/}((\CONS~x~y),\sigma,\Lfonly)$.   Let
$\alpha \in \sigma$.  The symbol \bcar(\bcdr) merely marks a
place in $\alpha$ where  the original \CONS\ rule would have
erased an immediately following \acar(\acdr), or, in absence
of such a symbol, would have dropped $\alpha$ itself.  Since
the  application of  any  rule in  \Lfunonly$'$ merely  adds
symbols at the beginning  of $\alpha$, the markers and other
symbols in $\alpha$ are  propagated to other dependent parts
of program in  their same relative positions.  Consequently,
the  erasure carried  out at  the end  of the  analysis with
$\stackrel{*}{\hookrightarrow}$  gives  the  same result  as
obtained through \Lfunonly.
...
We shall consider  the program in Figure~\ref{fig:examp-pgm}
as a running example; this is the ANF version of the program
in Figure~\ref{fig:mot-example}.

%%AM no longer used: In  what follows, we shall overload the semantics
%% of   \Lfunonly\  so  that   \Lfun{e}{\sigma}{\Lfonly}  additionally
%% records  the liveness  environment at  every sub-expression  of $e$
