...
\item \emph{GC}  for each  $x \in domain(\rho)$,  we set  $\rho(x)$ to
  $\bot$ iff $x.\epsilon \notin \Lv$.  Similarly, for each stack entry
  $(\rho,\ell,e,\sigma)$  in  $S$  with $\Lv'$  as  the  corresponding
  liveness environment in  $\mathsf{SL}$, and finally for  each $x \in
  domain(\rho)$  for some  closure in  the heap,  we set  $\rho(x)$ to
  $\bot$ iff $x.\epsilon \notin \Lv'$ corresponding to the closure.
