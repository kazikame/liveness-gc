...
&& \;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;
                                        ...
                                        \cred{\pa.\cbar\bcar\Lf{\length}{\mbox{1}}{\sigma}} \rbrace
                               ] \\
...
\cbar\} as terminal symbols and \Df{\length}{1} as the sole
non-terminal.
...
\end{eqnarray*}
As examples, the liveness of \Lanv{1}{\pl} and \Lanv{9}{\pc} in terms
...
\Lanv{1}{\pl} &= \cbar\sigma_{\length} \cup \acdr\Df{\length}{1}\cbar\sigma_{\length}
 ...
 \\
\Lanv{9}{\pc} &= \Df{\length}{1}\sigma_{\mathit{\!all}}
\end{align*}
...
&\rightarrow&
\cred{\epsilon}  \mid \cbar\langle{\sigma_{\length}}\rangle
\end{eqnarray*}
...
\label{sec:NFA-approx}
 ...
 approximate   a  CFG   to   a  {\em   strongly  regular\/}   grammar.
 \cred{The grammar for the productions we considered after transformation is,
 \begin{eqnarray*}
   \var{\Df{\length}{1}} &\rightarrow& \cbar\var{\Df{\length}{1*}} \mid
   \acdr\var{\Df{\length}{1}}
   \mid \cbar\var{\Df{\length}{1}}\\
   \var{\Df{\length}{1*}} &\rightarrow& \cbar\var{\Df{\length}{1*}} \mid \epsilon \\
   \langle {\sigma_{\length}} \rangle
   &\rightarrow&
   \cred{\epsilon}  \mid \cbar\langle{\sigma_{\length}}\rangle
 \end{eqnarray*}
 ...
 \comment{Rewrite  this after  constructing the  example: We  show the
   grammar for ????? after  the Mohri-Nederhof transformation. We pick
   the only  production that  is affected by  the transformation---the
   production    for     \Df{\append}{1}.     The    production    for
   \Uf{\append}{1},  while recursive, is  already in  strongly regular
...
otherwise it does not reach a final state.}
...
\lang{\nt}$, where {\nt} accepts forward paths only.  \comment{Rewrite
  the algorithm to handle \cbar  and update the example automata} Thus
...
termination and correctness  of th The algorithm repeatedly introduces  $\epsilon$ edges to bypass a pair
of consecutive edges labelled  \bcar\acar\ or \bcdr\acdr.  The process
is continued till  a fixed point is reached.  When  the fixed point is
reached,   the  resulting   NFA  contains   all   possible  reductions
corresponding to all the paths in the original NFA.  The proofs of the
...
reached,   the  resulting   NFA  contains   all   possible  reductions
corresponding to all the paths in the original NFA.  The proofs of the
termination and correctness  of th 
