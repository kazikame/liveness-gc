...
languages.   This  requires  changes  both  in  the  earlier  liveness
analysis and also in the garbage collection scheme required to reclaim
memory that is not live.
...
{\em Proof:}  Given a Turing machine  $TM$ and an input  $w$, we shall
construct an unrestricted  grammar $G$ of the kind  described above, so
that $TM$ will halt on $w$ if and only if $\epsilon \in \mathscr{L}(G)$.

We  shall  represent  the  configuration  of  $TM$  at  any  point  as
$w_L(\state,c)w_R$, where $w_L$ is the string on the left of the head,
$w_R$ is the string on the right  of the head, $c$ is the symbol under
the head and \state\ is the current state of the machine. For each
$(\state,c)$ combination of state and symbol, the grammar
will contain a nonterminal \nont{}{c}. Each step of the machine will
correspond to step of derivation based on the grammar.  In addition, we shall maintain
the following invariant between the configuration of $TM$ and the
...
\begin{quote}
  ...
  $\overline{w}_L$ is the same as $w_L$ but with each symbol $d$
  replaced with $\overline{d}$.
\end{quote}

Assume that the TM starts in a state $S_{init}$ with a tape $cw$ and the
head is positioned on the symbol $c$. Then the start non-terminal  of the
grammar $\nont{}{}$ is associated with the production $\nont{}{} \rightarrow \nont{init}{c}w$ and the
sentential form corresponding to the initial configuration is
$\nont{init}{c}w$. Further each transition of the turing machine is modeled by
productions as follows:

\begin{enumerate}
\item A transition $(S_i, c) \rightarrow (S_j,c',L)$ by two productions
  $\nont{i}{c} \rightarrow \acar \nont{j}{\acar}c'$ and $\nont{i}{c}
  \rightarrow \acdr \nont{j}{\acdr}c'$. 
\item A transition $(S_i, c) \rightarrow (S_j,c',R)$ by two productions
  $\nont{i}{c} \rightarrow c \nont{j}{\acar}\bcar$ and $\nont{i}{c}
  \rightarrow c \nont{j}{\acdr}\bcdr$. 
\end{enumerate}
The idea behind these productions is as follows: Assume that the
current sentential form is
\ldots\bcar\bcdr\nont{i}{\acar}\acar\acar\ldots and Starting from $(S_i,
c)$ if the head writes $c'$ and moves to the left, then the symbol
$c'$ is exposed to the right of the tape head. However, we do not know
which symbol that is under the head after the movement. Therefore we
generate two productions assuming that the symbols are \acar\ and
\bcar. If the guess is right, then to remove the symbol to the left in
the sentential form   
