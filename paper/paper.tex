...
eager language. An earlier  effort in efficiently garbage collecting
closures~\cite{Thomas19951}  used  a   similar  approach  where  every
closure  would maintain  a  sequence of  executable  code called  {\em
  preserver}  which  identified  exactly which  closure  variables  to
copy. The main focus of the  above work was also in garbage collecting
closures without using  an explicit stack. However, it not clear whether
their method could handle partially live data structures.
...
\added{
\begin{itemize}
\item {\color{Myblue}We  propose a liveness  based GC scheme  for a lazy  first order
