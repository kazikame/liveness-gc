...
explicitly  (for  example using  constructors)  or implicitly  (during
creation    of    closures).     However,   empirical    studies    on
...
their reachability from a set of memory locations called the {\em root
  set\/}. This results in unnecessary memory retention.

Programs written in languages having lazy semantics for evaluation put
an further demands on  memory as they require heap allocated
closures to be  carried from the point of  occurrence of an expression
to the point of (first) real use of that expression.
%% This   may   be   mitigated  by   strictness
%% analysis~\cite{some-refs} that  can statically determine  the patterns
%% of future usage  of an expression, thus allowing  some of the closures
%% to be  evaluated at the point  of the occurrence  of the corresponding
%% expression.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% MOTIVATING EXAMPLE
\newcommand{\nilfigure}
...
Using  a central notion  of demand,  we formulate  a context-sensitive
liveness  analysis  and  prove   it  correct.   We  use  a  0-CFA-like
conservative   approximation   to   annotate   each   allocation   and
function-call program point  with a finite-state automaton---which the
garbage-collector inspects to curtail reachability during marking. 


While this idea has been shown to be effective~\cite{asati14lgc} for a
first-order  eager  language,  a  straightforward  extension  of  this
earlier  technique  is  not   possible  in  lazy  languages,  where  a
heap-allocated   object  may  either   be  a   evaluated  data   or  a
closure. Since liveness is brought about by evaluation of closures and
given the  uncertainty in lazy  languages regarding the  program point
where this  evaluation takes  place, this introduces  complications in
the analysis itself.  Further,  since we want to extend liveness-based
collection to closures, we  track closures  to their
points of creation, from where we obtain their liveness. Since this is
done  at  runtime,  this  requires considerable  modification  to  the
earlier GC scheme.

%% We  previously  proposed an  interprocedural  liveness analysis  to
%% improve   garbage   collection   first   order  eager   subset   of
%% Scheme~\cite{asati14lgc}.   The current paper  extends the  work to
%% handle lazy evaluation semantics for the same language.

Experiments  confirm  the expected  performance  benefits.  The  early
evaluation   of   closures  decreases   the   memory   demands  of   a
program. Further,  liveness-based collection results in  a increase in
garbage  reclaimed  and  a   consequent  decrease  in  the  number  of
collections,  a  (some quantification)  decrease  in  the memory  size
required to run programs,  and reduced overall garbage collection time
for a majority of programs.
...
% the language.
Liveness analysis is  described in Section~\ref{sec:liveness} followed
by  a  sketch  of  a  correctness proof  relative  to  a  non-standard
...
The body  of a function ${\mathit  f}$ will be  denoted as $e_{\mathit
  f}$. for  analysis purposes it is convenient  to regard $e_\mainpgm$
as part of a function definition $(\DEFINE\ ({\tt main})\ e_\mainpgm)$
as in  C\@.  We  write $\pi\!:\!e$ to  associate the label  $\pi$ (not
part  of the  language  syntax)  with the  program  point just  before
expression $e$.
...
constituents) or a {\em closure}.  A  closure is a pair $(s, \rho)$ in
which $s$  is an unevaluated application  with the values  of its free
variables given by $\rho$.
...
(denoted  by  $\pi$ and  $\psi$).   The  liveness  at $\pi:e$  is  the
    ...
    \begin{boxedminipage}{.45\textwidth}
        ...
        \renewcommand{\arraystretch}{1}{
          \begin{uprogram}
            ...
            \UNL{5} \hspace*{.32cm} $\pi_4\!\!:\, (\RETURN~\psi_2:\pv)$
            \UNL{4} \hspace*{.29cm}    $\pi_5\!\!:\, $(\LET~\pu\  $\leftarrow$  (\CDR~\xl)  \IN
            \UNL{5} \hspace*{.34cm}   $\pi_6\!\!:\, $(\LET~\py\  $\leftarrow$  (\length~\pu)  \IN
            \UNL{6} \hspace*{.34cm} $\pi_7\!\!:\,
            $(\LET~\pz\ $\leftarrow$ (1~+~\py) \IN
            \UNL{7} \hspace*{.34cm} $\pi_8\!\!:\, (\RETURN~\psi_3:\pz)$)))))))
        \end{uprogram}}
        \renewcommand{\arraystretch}{1}{
	  \begin{uprogram}
	  \UNL{1} $(\DEFINE\ (\main)$
          \UNL{2} \!\!$\pi_9\!\!:\, (\LET\  \pa\  \leftarrow$ \ldots  \IN
	  \UNL{3}   \hspace*{.05cm}$\pi_{10}\!\!:\,      $ (\LET\ \pb\  $\leftarrow$  \ldots \IN 
          \UNL{4}   \hspace*{.15cm}    $\pi_{11}\!\!:\,
          $(\LET\ \pw\  $\leftarrow$  (\length\ \pb) \IN
          \UNL{5}  \hspace*{.25cm}  $\pi_{12}\!\!:\, (\RETURN~\psi_4:\pw)))))$
\end{uprogram}}
        %%} 
    \end{boxedminipage}
    ...
    \begin{boxedminipage}{.51\textwidth}
        ...
        $\mathcal{L}(e_{\length},\sigma,\Lfonly) = $ 
            \begin{tabular}{lc}            
              $[~ep_1 \mapsto \lbrace \px.\sigma_{\length}, \xl.\cbar\sigma_{\length} \rbrace ,~ep_2 \mapsto \lbrace \rbrace , $\\
              $~~ep_3 \mapsto \lbrace \pz.\sigma_{\length},
                \py.\cbar\sigma_{\length}, \pu.\Lf{\length}{\mbox{1}}{\cbar\sigma_{\length}}, $\\
              $~~~~~~~~~~~~~~\;\;
...
$ ~~~\pi_3 \mapsto \lbrace ep_2
                ...
                $~~~\pi_6 \mapsto \lbrace ep_3 \rbrace,  \pi_7 \mapsto
                  \lbrace ep_3\rbrace, \pi_8 \mapsto \lbrace ep_3 \rbrace~\rbrack$
              \end{tabular}\\
              ...
              $\cal{P_{\mbox{\tiny \main}}} = $
                \begin{tabular}{lc}
                  ...
                  $\pi_8 \mapsto \lbrace ep_4 \rbrace,~\psi_4 \mapsto \lbrace ep_4 \rbrace~\rbrack$  
                \end{tabular}
        \end{tabular}
...
\kern -3ex
\vspace{0.5in}
\caption{Example Program and its liveness maps.{\color{red} Fix the
    formatting of the table on the right}}\label{fig:mot-example2}    
\end{figure*}
...
To see the  consequences of this, consider an analysis  of the body of
$\length$ with a demand $\sigma$.  Clearly, this is also the demand on
the return value $z$ giving the liveness at the program point $\pi_8$
as $\{\pz.\sigma\}$.  The liveness at $\pi_8$ is propagated backwards
as in traditional liveness analysis.
...
\item In an eager language, \py\ would not have been live at $\pi_8$,
  ...
  the beginning  of the program $\pi_1$ (since  we do not  kill liveness).
\end{enumerate}
...
evaluation\footnote{Program   points  which  trigger   evaluation  are
  labelled  with   $\psi$  {\color  {red}  ---slightly   late  in  the
    day}}---\SIF\ conditions  and \RETURN\ expressions.   We call such
...
$\mathit f$ to
the set of  e-paths sharing the point.  If a  program point happens to
fall on  more than one such  e-path, then the  liveness environment of
the  program  point  is   the  variable-wise  union  of  the  liveness
environments of the individual e-paths.  In the example program, since
${\mathcal P}_{\length}(\pi_1) = \lbrace ep_1, ep_2, ep_3\rbrace$, the
liveness at $\pi_1$ is ${\cal  M}(ep_1) \cup {\cal M}(ep_2) \cup {\cal
  M}(ep_3)$, where ${\cal M} = \mathcal{L}(e_f, \sigma, \Lfonly)$.  In
the  sequel we shall  only define  $\mathcal{L}$.  The  formulation of
$\cal{P}$ is easy and we do not elaborate it any further.



Consider  the $\mathcal{L}$-rules for  {\LET}, {\SIF},  and {\RETURN}.
The  expression $(\RETURN~\psi:x)$  with a  demand $\sigma$  forces an
evaluation  of   $x$  and  hence  a  new   map  $[\mathit{ep}  \mapsto
  \{x.\sigma\}] $  needs to  be created.  The  map for  the expression
...
\\[1ex]
\mathcal{L}((\RETURN~\psi:x),\sigma,\Lfonly) &=& \lbrack \mathit{ep} \mapsto
...
\mathcal{L}((\SIF~x~e_1~e_2),\sigma,\Lfonly) &=&
        ...
        \lbrack\lbrace \mathit{ep} \mapsto  \{x.\epsilon\}\rbrace\rbrack,  & \mbox{if}~\sigma \ne \emptyset\\
...
\mathcal{L}(e,\sigma,\Lfonly),~\Lv \in \mathcal{M}(ep),~\mbox{and}~f~\mbox{is the new function}\\
...
\end{figure*}
 ...
 arbitrary number of ``unrollings'' of the definition\footnote{The $*$
   in the definition of ${\mathit f}$ represents  non-deterministic choice,
   but can be treated as a constant for purposes of analysis.}.  We pass to the
 demand transformer $\Lfonly_{\mathit f}$  of this function the demand
 arising out  of the  liveness of  $x$ at the  beginning of  $e$. This
 gives the liveness of the variables of $s$.
 

\mycomment{The function  $\mathcal{L}$ now gives  the (total) liveness
  of  an   expression  $e$.   The  cases  $\RETURN$   and  $\SIF$  are
...
consider $\Lanv{1}{x_i}$,  the  liveness of    the $i^{\mbox{\footnotesize  th}}$
...
\end{eqnarray}
where \Df{f}{i} are sets of  strings over the alphabet mentioned above.   Substituting the guessed form in
...
\cbar\} as the terminal symbols and \Df{\length}{1} as the sole non-terminal.
...
\begin{align*}
\Lan{1} &= \lbrace  \px.\sigma_{\length}, \xl.\cbar\sigma_{\length}, \pz.\sigma_{\length},\
              \py.\cbar\sigma_{\length}, \\ &~~~~~~~\pu.\Lf{\length}{\mbox{1}}{\cbar\sigma_{\length}}, \xl.\acdr\Lf{\length}{\mbox{1}}{\cbar\sigma_{\length}} \rbrace \\
\Lan{2} & =  \lbrace  \px.\sigma_{\length}, \xl.\cbar\sigma_{\length}, \pz.\sigma_{\length},\
                  \py.\cbar\sigma_{\length}, \\ &\pu.\Lf{\length}{\mbox{1}}{\cbar\sigma_{\length}}, \xl.\acdr\Lf{\length}{\mbox{1}}{\cbar\sigma_{\length}} \rbrace \\
\Lan{5} &= \lbrace \pu.\Lf{\length}{\mbox{1}}{\cbar\sigma_{\length}}, \xl.\acdr\Lf{\length}{\mbox{1}}{\cbar\sigma_{\length}}\rbrace \\
\Lan{8} &= \lbrace \pw.\sigma_{\mathit{\!all}}, \pb.\Lf{\length}{\mbox{1}}{\sigma_{\mathit{\!all}}}, \ldots  \rbrace 
\end{align*}
...
function calls, every \LET~ point is a potential GC point. The number of 
automata that needs to be stored is reduced by sharing automata along all 
