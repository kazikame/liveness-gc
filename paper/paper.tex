...
\bang\ has to occur then some variable which is dereferenced along the
evaluation path  must be set to  $\bot$, between its creation  and use
points.  This can  only happen  if the  demand on  the variable  being
dereferenced  was null.  From  the  lemma~\ref{lem:correctness} it  is
clear that this cannot happen.


The rest  of the proof depends  on converting a program  with function
calls to an equivalent program without function calls for which we 
refer the user to~\cite{asati14lgc}.
