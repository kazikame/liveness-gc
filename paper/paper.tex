...
\rightsquigarrow
$  $\rho', S, H[\rho'(z) := (\rho(x), \rho(y))], e$}     &  \sc{cons} \\
\hline
$H(\rho(x)) \mbox{ is } (l_1, l_2)$ & $\rho, (\rho', z, e)\!:\!S, H,
(\CAR~x)  \rightsquigarrow \rho', S, H[\rho'(z) := H(l_1)], e$      &
\sc{car-whnf} \\
\hline
...
$\rho', S, H[\rho'(z) := H(\rho'(x)) + H(\rho'(y))], e$}      &
\sc{prim-whnf} \\
\hline
...
e)\!:\!S, H,
(\SRETURN~x)  \rightsquigarrow \rho', S, H[\rho'(z) := v], e$ &
\sc{return-whnf}\\
\hline
...
%        \small
      \renewcommand{\arraystretch}{1.1}{
        \begin{uprogram}
          \UNL{1} (\DEFINE\ (\abigfunction~\py)
          \UNL{2} $(Create\ a\ big\ closure\ and\ return)$ 
          \end{uprogram}}
        \renewcommand{\arraystretch}{1.1}{
          \begin{uprogram}
...
%% \comment{
%% While exploring the activation record of a function, LGC uses the DFA
%% at  the latest  program  point traversed  in the function's body.
%% Thus, if  LGC
%% is invoked  at the program point   $\pi$,  it would  use  the
%% liveness  DFA at  $\pi$ itself for exploring the root set of the
...
%% call-chain,
%% the DFA at the point of call to  ${\mathit g}$ would be used.}
...
statisctics for  RGC and LGC  with reachability for  copying closures.
We report the  number of GC events, average number  of cells reclaimed
per  GC, average  number  of cells  touched per  GC,  total number  of
dragged cells  and total time  to perform  all collections.  It  is no
surprise  that  number  of  cells  reclaimed per  GC  is  higher  (and
consequently the  number of  GCs is lower)  for LGC  based collection.
LGC also reduces  the number of dragged  cells significantly. However,
the cost for LGC is that per  GC execution time is more for LGC, which
results in higher overall execution  time for garbage collection, even
with reduced  number of collections.   However, LGC run time  is still
competitive (slowdown within 5X of RGC  for worst case), and is better
for   3    benchmarks   (best    case   2X   speedup).     Note   that
\verb@gc_bench@~\cite{gc_bench}   is  a   synthestic  benchmark   that
allocates complete binary trees of  various sizes.  These binary trees
are  never used  by the  program. As  a result,  the benchmark  highly
favours LGC.   We have included  this benchmark for  completeness only
and the numbers for it  are not considered representative of practical
programs. All  our benchmarks were executed  on a desktop with  64 bit
Ubuntu 14.04, 8 core Intel(R) Core(TM) i7-4770 CPU @ 3.40GHz, L2 cache
of 8192 KB and 15922 MiB (16 GB) RAM.
...
languages~\ref{asati14lgc} the gap was  very narrow and almost touched
...
>>>>>>> 86509285950f09b0dc95515465513023c4b6fa40
...
A few observations: First observe that the transitions  which involving dereferencing and can lead to \bang are those which have a $H(\rho(x))$ term in their premise for some variable $x$.
