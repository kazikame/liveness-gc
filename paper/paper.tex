...
implemented a copying collector that uses the results of the liveness
...
the same program running with a reachability-based collector.
\end{abstract}
...
Any attempt to dereference a location with a null-liveness  
...
one for each non-terminal \var{\Lanv{x}{}} regarded as
...
~\cite{asati14lgc}. 
A major source of inefficiency in LGC is multiple traversals of
already copied heap cells. Since LGC does not mark the heap cells
after the first visit, the same cells can be repeatedly visited with
different liveness states. We have mitigated this problem by
implementing a heuristic which tags \CONS\ cells with the set of
...
The important paper
by~\cite{Hirzel} investigates the impact of liveness on the
effectiveness of garbage collection. Their observation is that liveness
can significantly impact garbage collection but only when it is
...
nullifying statements in Java programs. ~\cite{ran.shaham-sas03} is
another work which constructs an automaton modelling heap accesses and
checks temporal properties like liveness against the model.  
However both the approaches are restricted to intraprocedural scope.

In the space of functional languages, efforts towards improving
space efficiency using compile-time  analysis are: rewriting
methods                             such                            as
...
%%   evaluation of  $e$.  The  information in  the continuation frames on
%%   the   stack  $S$ are  also
%%   similarly augmented with their  demands.  Thus a stacked
