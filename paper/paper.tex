...
\hline
\hline
          &\makecell{ $\rho, (\rho', x, e)\!:\!S, H, \kappa$ \\ $\rightsquigarrow \rho', S, H[\rho'(x) := \kappa], e$ }   &  \sc{const}
\\
\hline
          & \makecell{$\rho, (\rho', z, e)\!:\!S, H, (\CONS~x~y)$ \\ $\rightsquigarrow  \rho', S, H[z := (H(\rho(x)),H(\rho(y)))], e$}     &  \sc{cons} \\
\hline
$H(\rho(x)) \mbox{ is } (d_1, d_2)$ & \makecell{$\rho, (\rho', z, e)\!:\!S, H, (\CAR~x)$ \\ $ \rightsquigarrow \rho', S, H[\rho(z) := d_1], e$}      &
\sc{car-whnf} \\
\hline
$H(\rho(x)) \mbox{ is } \langle s, \rho'\rangle$ &\makecell{ $\rho, S, H, (\CAR~x)$ \\ $\rightsquigarrow \rho', (\rho, x, (\CAR~x))\!:\!S, H, s$}      &
\sc{car-clo}
\\
\hline
$H(\rho(x)), H(\rho(y)) \in \mathbb{N}$
 & \makecell{$\rho, (\rho', z, e)\!:\!S, H, (+~x~y)$ \\  $\rightsquigarrow \rho', S, H[\rho(z) \mapsto H(\rho(x)) + H(\rho(y))], e$}      &
\sc{prim-whnf} \\
\hline
$H(\rho(x)) \mbox{ is } \langle s, \rho'\rangle$ &\makecell{$\rho, S, H, (+~x~y)$ \\ $\rightsquigarrow \rho', (\rho, x, (+~x~y))\!:\!S, H, s$}      &
\sc{prim-1-clo} \\
\hline
$H(\rho(y)) \mbox{ is } \langle s, \rho'\rangle $ & \makecell{$\rho, S, H, (+~x~y)$ \\ $\rightsquigarrow \rho', (\rho, x, (+~x~y))\!:\!S, H, s$}      &
\sc{prim-2-clo} \\
\hline
{$\mathit{f}~\mbox{defined as}$
$~(\DEFINE~(f~\myvec{y})~e_{\mathit{f}})$}  & \makecell{$\rho, S, H, (f~\myvec{x})$ \\ $\rightsquigarrow [\myvec{y} \mapsto \rho(\myvec{x})], S, H, e_{\mathit{f}}$}      &
\sc{funcall} \\
\hline
...
\sc{let} \\
\hline
$H(\rho(x)) \ne 0$ & \makecell{$\rho, S, H, (\SIF~x~e_1~e_2), ep, \sigma$ \\ $\rightsquigarrow \rho, S, H,  e_1, ep, \sigma$} & \sc{if-true} \\
\hline
$H(\rho(x)) = 0$ & \makecell{$\rho, S, H, (\SIF~x~e_1~e_2), ep, \sigma$ \\   $\rightsquigarrow
\rho, S, H,  e_2, ep, \sigma$} & \sc{if-false} \\
\hline
$H(\rho(x)) = \langle s, \rho' \rangle $ & \makecell{$\rho, S, H,  (\SIF~x~e_1~e_2), ep , \sigma$ \\ $\rightsquigarrow
\rho', (\rho, x, (\SIF~x~e_1~e_2), ep, \sigma)\!:\!S, H,  s, ep_{if}, \sigma$}
&
\sc{if-clo} \\
\hline
{$H(\rho(x))~\mbox{is}$ $\mbox{whnf with value}~v$}& \makecell{$\rho, (\rho', z, e, ep, \sigma)\!:\!S, H, (\SRETURN~x), ep_{return}, \sigma$ \\ $\rightsquigarrow \rho', S, H[\rho(z) \mapsto v], e, ep, \sigma$} &
\sc{return-whnf}\\
\hline
$H(\rho(x)) = \langle s, \rho' \rangle $ & \makecell{$\rho, S, H, (\SRETURN~x), ep, \sigma$ \\ $
  \rightsquigarrow$
$\rho',~ (\rho, x, (\SRETURN~x), ep, \sigma)\!:\!S, H,  s, ep_{return}, \sigma$} &
\sc{return-clo} \\
\hline
\end{tabular}
