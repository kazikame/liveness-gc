...
is given by $\Lv_\mathit{x_{\pi}}$.
 ...
 As we  shall see in section??,  the liveness of the  closure variable
 $x_{\pi}$ is recorded along with the closure so that the GC can refer
 to it during garbage collection.
}
%} %comment 
...
\item      We     augment      the      standard     semantics      in
  ...
  of  each  \LET\  expression.   {\color{Myblue}Note  that  memory  is
    allocated only during execution  of \LET\ expressions}. During GC,
  we  track each  reference  in the  root  set and  heap  that is  not
  declared  live by  our analysis.   Any attempt  to dereference  such
  references results in the transition system entering a special state
  denoted   \bang.   We   call  the   semantics  after   augmentation,
  \emph{minefield semantics}.
...
\item  $GC(\rho,  S,  \heap,  e, \sigma)$ models  a  liveness-based  garbage
 ...
 \item  Consider  the following  environments,  1)  environment in  the
  ...
  these  environments  we  can   calculate  a  corresponding  liveness
  environment using the $e\ and\ \sigma$.
...
\bang\  state.    When  $n$   is  0,   the  state   is  $([\;]_\rho,\,
(\rho_\mathit{init},   \,  \ell_{\ans}   ,\,  (\print~\ans)):[\;]_{S},
        [\;]_{\heap},\, (\mainpgm))$.  Since the  call to $GC$ in this
        state  does   nothing,  we  ju1st   have  to  show   that  the
        $\rightsquigarrow$  transition cannot  enter  a \bang\  state.
        Since our  programs are in  ANF, $e_{\mainpgm}$ can only  be a
        $\LET$  expression.   A  {\sc   let}  step  does  not  involve
        dereferencing, and thus cannot result in a \bang.
...
inserted by the immediately preceding  garbage collection or the GC of
an earlier minefield  step.  However the demand $\sigma'$  on basis of
which an earlier  collection would have inserted a  $\bot$, would have
included the  current demand $\sigma$.   Thus, if the  earlier garbage
collection had inserted a $\bot$, so would the current GC.  Thus it is
enough to  consider the $\rightsquigarrow$  along with the  current GC
and show that it does not lead to a \bang.
...
liveness  are checked  against an  automaton modelling  heap accesses.
...
marking, explicit deallocation and desctructive allocation. An
