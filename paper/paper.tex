...
We shall call  a unit allocatable memory as a {\em  cell}.  A cell may
hold  a  basic value,  or  the constructor  \CONS\,  or  (parts of)  a
closure.  Apart from information to  identify the cell type, each cell
has the space  for two fields. The second of the  fields may be empty,
when, for example, the cell holds a basic value or a closure involving
a unary operator. \cred{Should we say more about the representation}. 

\begin{figure*}[t!]
\label{fig:algos}
\hfill
\subfigure[]{\includegraphics[width=.27\linewidth]{lgc-scheme-1}}
\hfill
\subfigure[]{\includegraphics[width=.45\linewidth]{lgc-scheme-2}}
\hfill
\caption{(a) LGC with closure collection based on reachability  (b)
  LGC with liveness-based closure collection.}
...
book}.
Printing of atomic values is simple and does not trigger a garbage
collection.
In case of \CONS\ cells, the \CAR\ part is first printed and then the \CDR\
part.
In a lazy language, both the \CAR\ and the \CDR\ parts could be closures,
thus
...
is not part of the executing program. Since the print function makes
everything
reachable from a cell live we can copy all cells reachable from the
reference.


Consider the case where a \CONS\ cells is being printed and a garbage
collection
is triggered when printing the \CDR\ part. In this case, since the refernce
to the
\CONS\ cells is still on the print stack both the \CAR\ and the \CDR\ parts
are
reachable but we know that the \CAR\ part has already been printed and can be
safely garbage collected. A liveness analysis of the print function would
have
...
has not been printed, a reference to the \CAR\ part will also be on the stack
but
the \CDR\ part has to be explicitly copied.
In case the \CAR\ part has been printed, then a reference to the \CDR\ part
will
