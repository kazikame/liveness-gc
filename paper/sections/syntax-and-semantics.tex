...
% \paragraph{Inlining.}
{\color{blue}In spite of the ANF  restrictions it is still possible to
...
The  semantics of  expressions  (and applications)  are  given by  the
judgement form $\rho, \stk, \heap, e \rightarrow \rho', \stk', \heap',
e'$.  Here \stk\ is a stack  a list of continuation frames viewed as a
list.   Each continuation  is of  the form  $(\rho, x,  e)$  where the
location of $x$ is updated when the expression that is currently being
evaluated reaches WHNF, and $e$ is the next expression to be evaluated
in  the  environment  $\rho$.    The  start  state  is  $(\{\},[(\{\},
  \mainpgm,  \HALT)], \{\},  e_\mainpgm)$, and  the  program terminates
successfully on  reaching the  halt state $(\rho,[\,],  \heap, \HALT)$
with $\rho(\mainpgm)$ containing the  final result. Note that since we
...
using a static type system) lead to {\em stuck states} i.e. states not
covered by any rules in the small-step semantics.
