...
(define (gen n)
  (let ((n/2 (quotient n 2)))
    (let ((radicals (make-vector (+ n/2 1) '(H))))
...
(define (bcp-loop1 radicals rads1 lst)
  (if (null? rads1)
      lst
      (bcp-loop2 rads1 rads1
		 (bcp-loop1 radicals (cdr rads1) lst))))
...
(define (ccp-loop2 nc1 nc2 nc3 nc4 radicals rads1 lst)
  (if (null? rads1)
      lst
      (ccp-loop3 nc1 nc2 nc3 nc4 radicals rads1
		 (if (= nc1 nc2)
		     rads1
		     (vector-ref radicals nc2))
		 (ccp-loop2 nc1 nc2 nc3 nc4 radicals (cdr rads1) lst))))

(define (ccp-loop3 nc1 nc2 nc3 nc4 radicals rads1 rads2 lst)
  (if (null? rads2)
      lst
      (ccp-loop4 nc3 nc4 radicals rads1 rads2 
		 (if (= nc2 nc3)
		     rads2
		     (vector-ref radicals nc3))
		 (ccp-loop3 nc1 nc2 nc3 nc4 radicals rads1 (cdr rads2) lst))))

(define (ccp-loop4 nc3 nc4 radicals rads1 rads2 rads3 lst)
  (if (null? rads3)
      lst
      (ccp-loop5 radicals rads1 rads2 rads3
		 (if (= nc3 nc4)
		     rads3
		     (vector-ref radicals nc4))
		 (ccp-loop4 nc3 nc4 radicals rads1 rads2 (cdr rads3) lst))))

(define (ccp-loop5 radicals rads1 rads2 rads3 rads4 lst)
  (if (null? rads4)
      lst
      (cons (vector-5 'CCP
		      (car rads1)
		      ...
		      (car rads3)
		      (car rads4))
	    (ccp-loop5 radicals rads1 rads2 rads3 (cdr rads4) lst))))
